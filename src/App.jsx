...
import { Home, Users, FileText, Image, Target, Search, Plus, X, Edit2, Save, Activity, AlertCircle, Clock, DollarSign, TrendingUp, Calendar, Trash2, LogOut } from 'lucide-react';
...
function AuthGate({ onLogin }) {
  ...
  function handleSubmit(e) {
    ...
    if (username === DEMO_USER && password === DEMO_PASS) {
      onLogin({ name: username, provider: 'password' });
      return;
    }
    ...
    <div className="min-h-screen bg-black flex items-center justify-center p-4" style={{fontFamily:'system-ui,sans-serif'}}>
      <div className="w-full max-w-md bg-gray-950 border border-gray-800 rounded-2xl p-6">
        <div className="mb-6 text-center">
          ...
          <div className="text-2xl font-black tracking-widest text-white">UNMETERED</div>
          <div className="text-sm text-gray-500 mt-2">Sign in to Business Hub</div>
        </div>

        <form onSubmit={handleSubmit} className="space-y-3">
          <input value={username} onChange={(e)=>setUsername(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white" placeholder="Username" />
          <input type="password" value={password} onChange={(e)=>setPassword(e.target.value)} className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white" placeholder="Password" />
          {error && <p className="text-xs text-red-400">{error}</p>}
          <button type="submit" className="w-full py-2 rounded-lg text-sm font-semibold bg-white text-black hover:bg-gray-200">Sign in</button>
        </form>
        ...
        <div id="google-signin" className="flex justify-center min-h-10" />

        <div className="mt-6 text-xs text-gray-600 space-y-1">
          <p>Demo login: <span className="text-gray-400">admin / unmetered123</span></p>
          <p>To enable Google sign-in, replace <span className="text-gray-400">YOUR_GOOGLE_CLIENT_ID</span> in <code>src/App.jsx</code>.</p>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const [sessionUser, setSessionUser] = useLS('um2_auth_user', null);
  ...
  if (!sessionUser) {
    return <AuthGate onLogin={setSessionUser} />;
  }
    ...
    <div className="flex h-screen bg-black text-white" style={{fontFamily:'system-ui,sans-serif'}}>
      ...
      <div className="w-56 bg-gray-950 border-r border-gray-800 flex flex-col shrink-0">
        <div className="px-5 pt-6 pb-5 border-b border-gray-800">
          ...
          <div className="text-[11px] text-gray-600 mt-3 truncate">Signed in: {sessionUser?.name || 'User'}</div>
        </div>
        ...
        <div className="px-3 pb-5 pt-2 border-t border-gray-800 space-y-0.5">
          ...
          <button onClick={()=>setSessionUser(null)} className="w-full flex items-center gap-3 px-3 py-2 text-gray-600 hover:text-red-400 hover:bg-gray-800/60 rounded-xl text-xs transition-all"><LogOut size={15}/>Sign out</button>
        </div>
      </div>
